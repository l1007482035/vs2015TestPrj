...
{
    m_header.contentType().set("text/plain");
}
...
: TextEntity(text,charset)
{
    m_header.contentType().set("text/enriched");
}

TextHtml::TextHtml(const string& text)
: TextEntity(text)
{
	m_header.contentType("text/html");
}
TextHtml::TextHtml(const string& text, const string& charset)
: TextEntity(text,charset)
{
	m_header.contentType().set("text/html");
}
TextHtml::TextHtml(const std::string& text, const std::string& charset, const std::string& ContentTransferEncoding)
: TextEntity(text,charset)
{
#if 0	//此处会覆盖父类的构造函数【TextEntity(text,charset)】中的charset参数。
	m_header.contentType("text/html");
#else
	m_header.contentType().set("text/html");
#endif
	m_header.contentTransferEncoding(ContentTransferEncoding);
}
