	   ...
	   size_t     length)		/* I - Number of bytes to write */
{
  ...
  //DEBUG_printf(("=====httpWrite2=%s",buffer));
          ...
          size_t length)		/* I - Maximum bytes to read */
{
  ...
  printf("http=%p,buffer=%p,length=%d\n", (void *)http, (void *)buffer, length);
  ...
  do
  {
	...
	{//lzj						 
		char *pBuff = (char*)malloc(bytes + 1);
		memset(pBuff, 0x0, bytes + 1);
		for (int i = 0; i < bytes; i++)
		{
			sprintf(pBuff + i, "%c", buffer[i]);
		}
		DEBUG_printf(("======http_read,pBuff=%s", pBuff));
		free(pBuff);
		pBuff = NULL;
	}
  }
                   ...
                   size_t length)	/* I - Maximum bytes to read */
{
  ...
  //lzj
//   char *pBufff = (char*)malloc(bytes+1);
//   memset(pBufff, 0x0, bytes + 1);
//   for (int i = 0;i < bytes;i++)
//   {
// 	  sprintf(pBufff + i, buffer[i]);
//   }
//   DEBUG_printf(("pBufff=%s",pBufff));
//   free(pBufff);
//   pBufff = NULL;
  //end
	   ...
	   size_t     length)		/* I - Number of bytes to write */
{
  ...
  DEBUG_printf(("====================http_write,buffer=%s"), buffer);
