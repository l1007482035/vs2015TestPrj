     ...
     char *argv[])			/* I - Command-line arguments */
{
        ...
        switch (*opt)
	{
	  case '2' : /* -2 (enable 2-sided printing) */
          ...
          case 'A' : /* -A (enable authentication) */
			  WriteFileLog("======A,PAMService=0x%x",PAMService);
              if (!PAMService)
                PAMService = "cups";
	      break;
	  ...
	  case 'd' : /* -d spool-directory */
	      i ++;
		  WriteFileLog("=====-d,i=%d,argc=%d",i, argc);
	      if (i >= argc)
	        usage(1);
		  WriteFileLog("=======-d,2");
  ...
  */
  ...
  WriteFileLog("create_printer,servername=%s,serverport=%d,name=%s,location=%s,icon=%s,subtypes=%s,directory=%s,command=%s,device_uri=%s,output_format=%s"
	  , servername, serverport, name, location, icon, subtypes, directory, command, device_uri, output_format);
  printer = create_printer(servername, serverport, name, location, icon, docformats, subtypes, directory, command, device_uri, output_format, attrs);
  if (printer == NULL)
    return (1);
...
{
	char * data = client->http->data;
	WriteFileLog("====data=%s", data);
	const char		*authorization1;	/* Pointer into Authorization string */
	const char		*authorization2;
	const char		*authorization3;
	authorization1 = httpGetField(client->http, HTTP_FIELD_AUTHORIZATION);
	authorization2 = httpGetField(client->http, HTTP_FIELD_WWW_AUTHENTICATE);
	authorization3 = httpGetField(client->http, HTTP_FIELD_AUTHENTICATION_INFO);
	char *pStr1 = httpGetField(client->http, HTTP_FIELD_DATE);
	char *pStr2 = httpGetField(client->http, HTTP_FIELD_HOST);
	char *pStr3 = httpGetField(client->http, HTTP_FIELD_ACCEPT_LANGUAGE);
	char *pStr6 = httpGetField(client->http, HTTP_FIELD_CONTENT_ENCODING);
	WriteFileLog("=======authorization=%s,authorization2=%s,authorization3=%s", authorization1, authorization2, authorization3);
	WriteFileLog("=======pStr1=%s,pStr2=%s,pStr3=%s,pStr6=%s", pStr1, pStr2, pStr3, pStr6);

	char* pStr4 = client->http->_authstring;
	char* pStr5 = client->http->authstring;
	WriteFileLog("====pStr4=%s,pStr5=%s", pStr4, pStr5);
                ...
                int        family)	/* I - Address family */
{
  ...
  WriteFileLog("create_listener,name=%s,port=%d,family=%d", name, port, family);
    ...
    ipp_t        *attrs)		/* I - Capability attributes */
{
  ...
  */
  //WriteFileLog("create_printer,1,uri=%s", uri);
  //httpAssembleURI(HTTP_URI_CODING_ALL, uri, sizeof(uri), "ipp", NULL, printer->hostname, printer->port, "/ipp/print"); //todol

  httpAssembleURI(HTTP_URI_CODING_ALL, uri, sizeof(uri), "ipp", NULL, printer->hostname, printer->port, "/printers/Pantum_M7100DW_Series_PCL62");

  WriteFileLog("create_printer,2,uri=%s", uri);
...
#ifdef HAVE_SSL
  WriteFileLog("create_printer,7,uri=%s,securi=%s", uri, securi);
  uris[0] = uri;
  uris[1] = securi;
    ...
    ippeve_job_t    *job)		/* I - Job */
{
  ...
  */
  ...
  WriteFileLog("========finish_document_data");
...
finish_document_uri(
    ...
    ippeve_job_t    *job)		/* I - Job */
{
	WriteFileLog("finish_document_uri,entry");
  ...
  */
  ...
  else
  {
	...
	WriteFileLog("========finish_document_uri");
    ...
    ippeve_client_t *client)		/* I - Client */
{
  ...
  if (ippContainsString(actions, "display"))
    WriteFileLog("IDENTIFY from %s: %s", client->hostname, message ? ippGetString(message, 0, NULL) : "No message supplied");
...
process_http(ippeve_client_t *client)	/* I - Client connection */
{
  ...
  */
  ...
  //for test begin lzj
  http_t *pHttp = client->http;
  char* pData = pHttp->data;
  char* pData2 = pHttp->buffer;
  WriteFileLog("======process_http,pData=%s,pData2=%s", pData, pData2);
 
  for (int i =0;i < HTTP_FIELD_MAX;i++)
  {
	char* pFileld =  client->http->fields[i];
	WriteFileLog("=============pFileld=%s,i=%d", pFileld,i);

  }
  char* pAuth = httpGetField(client->http, HTTP_FIELD_AUTHORIZATION);
  WriteFileLog("=====pAuth=%s", pAuth);
  if (strlen(pAuth) > 0)
  {
	  respond_http(client, 100, NULL, NULL, 0);
  }
  ...
  switch (client->operation)
  {
	...
	*/
	...
	else
	{
	  ...
	  */
		WriteFileLog("process_http,调用,authenticate_request");
  ...
  else
  {
	...
	*/
	...
	int		port;		/* Port number in URI */
		...
		{
			WriteFileLog("================process ipp,name=%s,resource=%s", name, resource);
			WriteFileLog("================process ipp,%s %s not found", name, ippGetString(uri, 0, NULL));
        ...
        {
		//WriteFileLog("process_ipp,调用,authenticate_request");
          httpFlush(client->http);
    ...
    const char       *subtypes)		/* I - Service subtype(s) */
{
  ...
  uint32_t		ninterface;	/* Interface index */
...
#  endif /* HAVE_SSL */
  ...
  TXTRecordSetValue(&ipp_txt, "air", 17, "username,password"); //lzj

#else
  char *pStr = "1";
  TXTRecordSetValue(&ipp_txt, "txtvers", strlen(pStr), pStr);

  pStr = "1";
  TXTRecordSetValue(&ipp_txt, "qtotal", strlen(pStr), pStr);

  pStr = "printers/Pantum_M7100DW_Series_PCL6-12";
  TXTRecordSetValue(&ipp_txt, "rp", strlen(pStr), pStr);

  pStr = "Pantum M7100DW Series PCL6";
  TXTRecordSetValue(&ipp_txt, "ty", strlen(pStr), pStr);

  pStr = "http://WIN-5I4V32F5962:631/";
  TXTRecordSetValue(&ipp_txt, "adminurl", strlen(pStr), pStr);

  pStr = "";
  TXTRecordSetValue(&ipp_txt, "note", strlen(pStr), pStr);

  pStr = "0";
  TXTRecordSetValue(&ipp_txt, "priority", strlen(pStr), pStr);

  pStr = "Pantum M7100DW Series PCL6";
  TXTRecordSetValue(&ipp_txt, "product", strlen(pStr), pStr);

  pStr = "T";
  TXTRecordSetValue(&ipp_txt, "WINDOWS", strlen(pStr), pStr);

  pStr = "T";
  TXTRecordSetValue(&ipp_txt, "Transparent", strlen(pStr), pStr);

  pStr = "T";
  TXTRecordSetValue(&ipp_txt, "Binary", strlen(pStr), pStr);

  pStr = "F";
  TXTRecordSetValue(&ipp_txt, "Color", strlen(pStr), pStr);

  pStr = "T";
  TXTRecordSetValue(&ipp_txt, "Duplex", strlen(pStr), pStr);

  pStr = "F";
  TXTRecordSetValue(&ipp_txt, "Staple", strlen(pStr), pStr);

  pStr = "T";
  TXTRecordSetValue(&ipp_txt, "Copies", strlen(pStr), pStr);

  pStr = "F";
  TXTRecordSetValue(&ipp_txt, "Collate", strlen(pStr), pStr);

  pStr = "F";
  TXTRecordSetValue(&ipp_txt, "Punch", strlen(pStr), pStr);

  pStr = "F";
  TXTRecordSetValue(&ipp_txt, "Bind", strlen(pStr), pStr);

  pStr = "F";
  TXTRecordSetValue(&ipp_txt, "Sort", strlen(pStr), pStr);

  pStr = "F";
  TXTRecordSetValue(&ipp_txt, "Scan", strlen(pStr), pStr);

  pStr = "application/pdf,image/jpeg,image/urf";
  TXTRecordSetValue(&ipp_txt, "pdl", strlen(pStr), pStr);

  pStr = "W8,DM1,CP255,RS600";
  TXTRecordSetValue(&ipp_txt, "URF", strlen(pStr), pStr);
  //lzj
   pStr = "username,password";
  TXTRecordSetValue(&ipp_txt, "air", strlen(pStr), pStr);
  ...
  */
  ...
  ninterface = !strcmp(printer->hostname, "localhost") ? kDNSServiceInterfaceIndexLocalOnly : kDNSServiceInterfaceIndexAny;

  printer->printer_ref = DNSSDMaster;

  WriteFileLog("===========DNSServiceRegister,1");
#if 0 //lzj
  if ((error = DNSServiceRegister(&(printer->printer_ref), kDNSServiceFlagsShareConnection, ninterface, printer->dnssd_name, "_printer._tcp", NULL /* domain */, NULL /* host */, 0 /* port */, 0 /* txtLen */, NULL /* txtRecord */, (DNSServiceRegisterReply)dnssd_callback, printer)) != kDNSServiceErr_NoError)
  {
    _cupsLangPrintf(stderr, _("Unable to register \"%s.%s\": %d"), printer->dnssd_name, "_printer._tcp", error);
    return (0);
  }
#endif
  WriteFileLog("===========DNSServiceRegister,2");
 /*
  ...
  */
  ...
  WriteFileLog("===========DNSServiceRegister,3,regtype=%s", regtype);

//   if ((error = DNSServiceRegister(&(printer->ipp_ref), kDNSServiceFlagsShareConnection, ninterface, printer->dnssd_name, regtype, NULL /* domain */, NULL /* host */, htons(printer->port), TXTRecordGetLength(&ipp_txt), TXTRecordGetBytesPtr(&ipp_txt), (DNSServiceRegisterReply)dnssd_callback, printer)) != kDNSServiceErr_NoError)
//   {
//     _cupsLangPrintf(stderr, _("Unable to register \"%s.%s\": %d"), printer->dnssd_name, regtype, error);
//     return (0);
//   }

//   if ((error = DNSServiceRegister(&(printer->ipp_ref), kDNSServiceFlagsShareConnection, ninterface, printer->dnssd_name, regtype, "local" /* domain */, NULL /* host */, htons(printer->port), TXTRecordGetLength(&ipp_txt), TXTRecordGetBytesPtr(&ipp_txt), (DNSServiceRegisterReply)dnssd_callback, printer)) != kDNSServiceErr_NoError)
//   {
// 	  _cupsLangPrintf(stderr, _("Unable to register \"%s.%s\": %d"), printer->dnssd_name, regtype, error);
// 	  return (0);
//   }

  if ((error = DNSServiceRegister(&(printer->ipp_ref), kDNSServiceFlagsShareConnection, ninterface, "Pantum M7100DW Series PCL6-12 @ WIN-5I4V32F5962", regtype, "local" /* domain */, NULL /* host */, htons(printer->port), TXTRecordGetLength(&ipp_txt), TXTRecordGetBytesPtr(&ipp_txt), (DNSServiceRegisterReply)dnssd_callback, printer)) != kDNSServiceErr_NoError)
  {
	  _cupsLangPrintf(stderr, _("Unable to register \"%s.%s\": %d"), printer->dnssd_name, regtype, error);
	  return (0);
  }
...
#  ifdef HAVE_SSL
  ...
  */
  ...
  WriteFileLog("===========DNSServiceRegister,4,regtype=%s", regtype);
#if 0 //lzj
  if ((error = DNSServiceRegister(&(printer->ipps_ref), kDNSServiceFlagsShareConnection, ninterface, printer->dnssd_name, regtype, NULL /* domain */, NULL /* host */, htons(printer->port), TXTRecordGetLength(&ipp_txt), TXTRecordGetBytesPtr(&ipp_txt), (DNSServiceRegisterReply)dnssd_callback, printer)) != kDNSServiceErr_NoError)
  {
    _cupsLangPrintf(stderr, _("Unable to register \"%s.%s\": %d"), printer->dnssd_name, regtype, error);
    return (0);
  }
...
#  endif /* HAVE_SSL */
  ...
  */
  ...
  WriteFileLog("===========DNSServiceRegister,5");
#if 0 //lzj
  if ((error = DNSServiceRegister(&(printer->http_ref), kDNSServiceFlagsShareConnection, ninterface, printer->dnssd_name, "_http._tcp,_printer", NULL /* domain */, NULL /* host */, htons(printer->port), 0 /* txtLen */, NULL /* txtRecord */, (DNSServiceRegisterReply)dnssd_callback, printer)) != kDNSServiceErr_NoError)
  {
    ...
    size_t        length)		/* I - Length of response */
{
  ...
  */
  ...
  WriteFileLog("================respond_http,code=%d", code);
  char* pAuth = httpGetField(client->http, HTTP_FIELD_AUTHORIZATION);
  WriteFileLog("======pAuth=%s", pAuth);
#if 1
  if (strlen(pAuth) <= 0)
