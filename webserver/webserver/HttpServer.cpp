...
int CHttpServer::GetResponseContentLength()
{
	...
	else if (m_bIsDynamicPage)
	{
		if (HTTP_GET == m_eMethod || HTTP_POST == m_eMethod)
		{
			...
			if (!CreateProcess(NULL, W_HTTP_CGIBIN_WINDOWS, NULL, NULL, TRUE, NULL, NULL, NULL, &siStartInfo, &ProcInfo))
				return 0;    
...
int CHttpServer::HttpParser(const char* pHttpData, int nDataLen)
{
	...
	if (m_eHttpParserStatus < HTTP_PARSER_REQUESTLINE)
	{
...
int CHttpServer::HttpResponseHeader(int nStatusCode, struct tagHttpResponseData* pHttpResponseData)
{
	...
	printf("======nStatusCode=%d,statues name=%s\n", nStatusCode,GetStatusCodeName(nStatusCode));
	const char* pVal = GetHeaderValueForKey("Authorization");
	printf("======pVal=%x\n", pVal);
	if (!pVal)
	{
		if (nStatusCode == W_HTTP_OK)
		{
			W_HTTP_SNPRINTF_FORMAT(m_szResponseHeader + nSumWriteLen, nPlanWriteLen, nLeftWriteLen, "HTTP/1.0 %d\r\n", W_HTTP_UNAUTHORIZED, GetStatusCodeName(W_HTTP_UNAUTHORIZED));
		}
	}
	else
	{
		W_HTTP_SNPRINTF_FORMAT(m_szResponseHeader + nSumWriteLen, nPlanWriteLen, nLeftWriteLen, "HTTP/1.0 %d\r\n", W_HTTP_OK, GetStatusCodeName(W_HTTP_OK));
	}
	...
	//W_HTTP_SNPRINTF_FORMAT(m_szResponseHeader + nSumWriteLen, nPlanWriteLen, nLeftWriteLen, "HTTP/1.0 401 Authorization Required\r\n");
	...
	if (bIsKeepAlive)
	{
		...
		if (!pVal)
		{
			W_HTTP_SNPRINTF_FORMAT(m_szResponseHeader + nSumWriteLen, nPlanWriteLen, nLeftWriteLen, "WWW-Authenticate: Basic realm=\"Secure Area\"\r\n");
			RESPONSE_HEADER_CHECK(nPlanWriteLen, nLeftWriteLen, nSumWriteLen);
		}
		else
		{
...
#ifdef _MSC_VER
	HANDLE hFD = CreateFile(m_szUriPath, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
