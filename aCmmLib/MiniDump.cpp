...
//Copyright (C)2002-2006 Merkur ( strEmail.Format(_T("%s@%s", "devteam"), "emule-project.net") / http://www.emule-project.net )
//
		...
		if (pfnMiniDumpWriteDump)
		{
			...
			//if (MessageBox(NULL, _T("eMule crashed :-(\r\n\r\nA diagnostic file can be created which will help the author to resolve this problem. This file will be saved on your Disk (and not sent).\r\n\r\nDo you want to create this file now?"), m_szAppName, MB_ICONSTOP | MB_YESNO) == IDYES)
			{
				...
				if (hFile != INVALID_HANDLE_VALUE)
				{
					...
					BOOL bOK = (*pfnMiniDumpWriteDump)(GetCurrentProcess(), GetCurrentProcessId(), hFile, MiniDumpWithIndirectlyReferencedMemory, &ExInfo, NULL, NULL);
					if (bOK)
					{
						// Do *NOT* localize that string (in fact, do not use MFC to load it)!
						...
						//_sntprintf(szResult, ARRSIZE(szResult), _T("%s程序出现错误已中止运行。\r\n\r\n我们已经创建了一个错误报告并保存在\"%s\"。\r\n您可以将它发送给我们，以帮助我们找出问题。"), pszFileName, szDumpPath);
						_sntprintf(szResult, ARRSIZE(szResult), CFFS(IDS_MINIDUMP_STR), pszFileName, szDumpPath);
...
void CMiniDumper::DumpNow(LPCTSTR szDumpPath)
{
	MINIDUMPWRITEDUMP pfnMiniDumpWriteDump = NULL;
	HMODULE hDll = GetDebugHelperDll((FARPROC*)&pfnMiniDumpWriteDump, true);
	...
	if(hDll && pfnMiniDumpWriteDump)
	{
		...
		if (hFile != INVALID_HANDLE_VALUE)
		{
			BOOL bOK = (*pfnMiniDumpWriteDump)(GetCurrentProcess(), GetCurrentProcessId(), hFile, MiniDumpNormal, NULL, NULL, NULL);
			CloseHandle(hFile);
		}
