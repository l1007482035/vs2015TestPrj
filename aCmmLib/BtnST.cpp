...
#if 0
	ASSERT(m_nTypeStyle != BS_OWNERDRAW);	// 因为使用CoolSkin，需要自绘的按钮，在resource editor里就要设成Owner Draw属性
#endif
...
// Do not use transparent buttons until you really need it (you have a bitmapped
...
//				If TRUE the control will be repainted.
//
void CButtonST::DrawTransparent(BOOL bRepaint, BOOL bSpecial)
{
	m_bDrawTransparent = TRUE;
	m_bSpecialBk = bSpecial;
	// Restore old bitmap (if any)
...
DWORD CButtonST::SetURL(LPCTSTR lpszURL)
{
	...
	memset(m_szURL, 0, _tcsclen(m_szURL)*sizeof(TCHAR));
...
void CButtonST::SetBkColor(COLORREF clrBk)
{
	m_clrBkColor = clrBk;
}
