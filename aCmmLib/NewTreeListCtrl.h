...
#define WM_TREELISTNODECHECKCHANGE		(WM_USER + 5003)	//节点状态变更
...
public:
	...
	//注意下面两个函数是否选中不不不不不不不不不不不不不不不不能用true或false判断。
	int  GetItemCheck(HTREEITEM hItem);
	void SetItemCheck(HTREEITEM hItem, int nCheck, BOOL bFromChild = FALSE);

	void OnBnClickedIcon(HTREEITEM hItem);//用户点击了图标
	void OnCheckItem(HTREEITEM hItem, int nCheck, BOOL bFromChild = FALSE);//某一项check变化

	void SetChildCheck(HTREEITEM hCurrent, int nCheck);//设置本节点的所有子节点
