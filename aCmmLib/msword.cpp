...
int GetCurrentWordVersion()
{
	int nVer = 0;
	HKEY hKey = HKEY_CLASSES_ROOT;
	LPCTSTR lpSubKey = _T("Word.Application\\CurVer");
	if (RegOpenKeyEx(hKey, lpSubKey, 0, KEY_QUERY_VALUE, &hKey) != ERROR_SUCCESS)
		return nVer;
	LPCTSTR lpValueName = NULL;
	DWORD dwType = REG_SZ;
	TCHAR szVer[MAX_PATH] = _T("");
	DWORD  cbValue = sizeof(szVer);
	if (RegQueryValueEx(hKey, lpValueName, NULL, &dwType, (LPBYTE)szVer, &cbValue) != ERROR_SUCCESS)
		return nVer;
	CString strVer = szVer;
	strVer = strVer.Right(strVer.GetLength() - strVer.ReverseFind('.') - 1);
	nVer = _tstoi(strVer);
	return nVer;
}
...
LPDISPATCH Documents::Open2000(VARIANT* FileName, VARIANT* ConfirmConversions, VARIANT* ReadOnly, VARIANT* AddToRecentFiles, VARIANT* PasswordDocument, VARIANT* PasswordTemplate, VARIANT* Revert, VARIANT* WritePasswordDocument, 
