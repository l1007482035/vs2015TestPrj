...
CString CHash::DoHash()
{
	...
	if (hashAlgo == CRC32)
	{
		...
		{
			crc32.StringCrc32((LPCTSTR)hashString, crc32r);
		}
		if (hashOperation == FILE_HASH)
		{
			crc32.FileCrc32Streams((LPCTSTR)hashFile, crc32r);
		}
...
CString CHash::GOSTHash()
{
	...
	if (hashOperation == FILE_HASH)
	{
		if (hashFile.IsEmpty()) return _T("");
		gosthash_reset(&m_gost);

		fileToHash = _tfopen(hashFile, _T("rb"));
...
CString CHash::MD5Hash()
{
	...
	if (hashOperation == STRING_HASH)
	{
		// Hashing a string, open the object, give it the text, and finalise it
		md5.MD5Init(&m_md5);
		md5.MD5Update(&m_md5, reinterpret_cast<unsigned char *>(hashString.GetBuffer(hashString.GetLength())), hashString.GetLength());
		md5.MD5Final(&m_md5);
	...
	if (hashOperation == FILE_HASH)
	{
		...
		if (md5.md5file(hashFile.GetBuffer(MAX_PATH), 0, &m_md5))
		{
			for (int i = 0; i < 16; i++)
			{
				char tmp[3];
...
//
//
void CHash::SetHashFile(LPCTSTR File)
{
	hashFile = File;
}
// End SetHashFile //////////////////////////////////////////////
