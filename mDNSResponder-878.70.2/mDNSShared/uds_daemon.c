...
{
	printf("register_service_instance,1\n");
...
{
	printf("handle_regservice_request,1\n");
    ...
    LogOperation("====1,%3d: DNSServiceRegister(%X, %d, \"%s\", \"%s\", \"%s\", \"%s\", %u) START PID[%d](%s)",
...
foundparent:;
    ...
    else LogMsgNoIdent("ProxyRecords: MISMATCH %d + %d = %d ¡Ù %d", ProxyA, ProxyD, ProxyA + ProxyD, m->ProxyRecords);
